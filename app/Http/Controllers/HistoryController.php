...
class HistoryController extends Controller
{
    ...
    private function get_data_for_table($companyArray = null)
    {
        ...
        $emergency_buzzer = $emergency_call = $call_received = $battery_low = $power_off = $power_on = $location_inquiry = 1;
        $connection_error = $connection_restore = 1;
        $positioning_with_at_true = $positioning_with_at_false = true;
        ...
        $company_device_histories = $this->search($companyArray, $date, $emergency_buzzer, $emergency_call, $call_received, $power_off, $power_on,
            $location_inquiry, $connection_error, $connection_restore, $battery_low, $device_model, $imei_number, $sb_billing_number, $group_name, $company_name, $msn, $command_center,
            $receiver_number, $positioning_with_at_true, $positioning_with_at_false);
    ...
    public function searchHistory(Request $request, HistoryQueryService $query_service)
    {
        if (session('company_device_history_history')) {
            ...
            $positioning_with_at_true = (bool) session('company_device_history_history')['positioning_with_at_true'];
            $positioning_with_at_false = (bool) session('company_device_history_history')['positioning_with_at_false'];
            //$dealt_with_at_true = (bool) session('company_device_history_history')['dealt_with_at_true'];
            //$dealt_with_at_false = (bool) session('company_device_history_history')['dealt_with_at_false'];
    ...
    public function search($companyArray, $date, $emergency_buzzer, $emergency_call, $call_received, $power_off, $power_on,
                           ...
                           $receiver_number, $positoning_with_at_true, $positioning_with_at_false){
        ...
        $query->where(function ($q) use ($positioning) {
